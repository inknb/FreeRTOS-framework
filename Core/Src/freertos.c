...
void StartDefaultTask(void const * argument)
{
  ...
  // --- 1. 定义任务参数 ---
  // 参数说明：任务名(chassisTask), 入口函数(Chassis_Task), 优先级(Normal), 实例数(0), 栈大小(512)
  // 注意：我们将栈大小改为了 512，因为 PID 运算涉及浮点数，默认的 128 可能会导致栈溢出死机！
  ...
  // 以下为云台代码
  osThreadDef(gimbalTask, Gimbal_Task, osPriorityHigh, 0, 512); // 云台优先级建议设高一点(High)
